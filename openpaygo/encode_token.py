...
class OPAYGOEncoder(object):
    ...
    def generate_standard_token(cls, starting_code, key, value, count,
        ...
        current_token = OPAYGOShared.put_base_in_token(
    ...
    def generate_extended_token(cls, starting_code, key, value, count, restricted_digit_set=False):
        ...
        current_token = OPAYGOSharedExtended.put_base_in_token(
            starting_code, token_base)
        new_count = count + 1
        for xn in range(0, new_count):
            current_token = OPAYGOSharedExtended.generate_next_token(
                current_token, key)
        final_token = OPAYGOSharedExtended.put_base_in_token(
            current_token, token_base)
        if restricted_digit_set:
            final_token = OPAYGOSharedExtended.convert_to_4_digit_token(
                final_token)
        return new_count, final_token
